  ...
  if(check_staff_permission('pro') && !in_array('pro',$Dpermissions,true)){
    ...
    if(check_staff_permission('admin')){
      //Prevent pro user from being deleted.
        ...
        permission_ungranted('users',$message);
        }else{
        $db->query("DELETE FROM users WHERE id = '$delete_id'");
...
if(isset($_GET['add']) || isset($_GET['edit'])){
  ...
  if(isset($_GET['edit'])){
        ...
        if(check_staff_permission('admin') && (in_array('pro',$Edit_permissions,true) ||  in_array('admin',$Edit_permissions,true))){
  ...
  if($_POST){
    if(isset($_GET['edit'])){
      $required = array('name', 'phone','email', 'permissions','last_login');
      $emailQuery = $db->query("SELECT * FROM users WHERE email='$email' AND id != $edit_id");
    }else{
      $required = array('name', 'phone','email', 'permissions','password', 'confirm');
    ...
    if(!empty($errors)){
      ...
      //add to database
        ...
        if(isset($_GET['edit'])){
          if(check_staff_permission('admin') && (in_array('pro',$editP,true) ||in_array('admin',$editP,true))){
            $message = "Please! You do not have clearance to asign that permission level";
            permission_ungranted('users?edit='.$edit_id.'',$message);
            break;
          }
          $insertSql = "UPDATE users SET full_name ='$name1', phone='$phone1', email ='$email1', permissions = '$permissions1',last_login ='$last_login'
          WHERE id='$edit_id'";
         ...
         $_SESSION['success_flash'] .= $name1. ' user has been updated!';
          ...
          if(check_staff_permission('admin') && in_array('pro',$addP,true)){
            $message = "Please! You do not have clearance to asign that permission level";
            permission_ungranted('users?add=1',$message);
            break;
          }
          $hashed =password_hash($password,PASSWORD_DEFAULT);
          $db->query("INSERT INTO users (full_name,phone,email,password,permissions) values('$name','$phone','$email','$hashed','$permissions')");
...
<form action="users?<?=((isset($_GET['edit']))?'edit='.$edit_id:'add=1');?>" method="post" enctype="multipart/form-data">
...
<a href="users?add=1" class="btn btn-lg btn-success pull-right" id="add-user-btn">Add New Staff</a>
          ...
          <tr>
            <td>
              ...
              <?php if($user['id'] != $user_data['id']){ ?>
                <a href="users?delete=<?=$user['id'];?>" class="btn btn-danger btn-xs" onclick="return deleletconfirm()"><span class ="glyphicon glyphicon-remove-sign"></span></a>
                <a href="users?edit=<?=$user['id'];?>" class="btn btn-info btn-xs" ><span class ="glyphicon glyphicon-pencil"></span></a>
              <?php }else {?>
                <a class="btn btn-success btn-xs" ><span class ="glyphicon glyphicon-user"><strong> Logged In</strong></span></a>
              <?php  }?>
            </td>
...
<hr>
<a href="customer" class="btn btn-lg btn-success pull-left" id="add-user-btn">View Registered Customer</a>
<hr>
