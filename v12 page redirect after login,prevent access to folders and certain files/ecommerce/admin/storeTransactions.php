...
$search = sanitize($_POST['searchTransactions']);
  $query1 = $db->query("SELECT * FROM transactions WHERE full_name LIKE '%".$search."%' OR email LIKE '%".$search."%' OR status LIKE '%".$search."%'");
  $rowCount = mysqli_num_rows($query1);
  $limit =$rowCount; //ignore pagination by setting limit to the returned rows
  $userTQuery = $db->query("SELECT * FROM transactions WHERE full_name LIKE '%".$search."%' OR email LIKE '%".$search."%' OR status LIKE '%".$search."%' ORDER BY txn_date DESC LIMIT $offset,$limit");
    if(is_null($userTQuery) || $rowCount ==0) {
...
$userTQuery = $db->query("SELECT * FROM transactions ORDER BY txn_date DESC LIMIT $offset,$limit");
...
$count = 1;
 ?>
<hr><h2 class="text-center">Transaction Table </h2><hr>
<div class="search-container">
  <form action="storeTransactions.php" method="post" required>
 ...
 <table class="table table-bordered table-striped table-condensed table-hover">
   <thead class= "bg-primary"><th>Status</th><th>Name</th><th>Email</th><th>Address</th><th>Charge ID</th>
     <th>
   ...
   <th>P_Type</th><th>P_Date</th><th>Sub total</th><th>Tax</th><th>Grand Total</th></thead>
   <tbody>
     <?php
     if($query->num_rows > 0){ ?>
         <div class="posts_list">
     <?php while($userT = $userTQuery->fetch_assoc()):
     ...
     <tr <?=(strcasecmp($userT['status'], 'Not Complete') == 0)? ' class="danger"':''?>>
       ...
       <td><?=money((int)$userT['sub_total']);?></td>
       <td><?=($userT['tax'] != 0)?money((int)$userT['tax']):'N/A';?></td>
       <td><?=money((int)$userT['grand_total']);?></td>
     </tr>
     <?php $count++; ?>
   <?php endwhile; }?>
   </tbody>
