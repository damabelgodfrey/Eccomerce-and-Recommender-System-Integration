...
function login($user_id,$user_username){
  ...
  $db->query("UPDATE users SET last_login = '$date' WHERE username = '$user_username'");
  $_SESSION['success_flash'] = $_SESSION['rdrurl'].'you are now logged in!';
      ...
      foreach($sizes as $size){
          //if($size['size'] == $itemp['size'] && $product_id == $edit_id){
            if($size['size'] == $edit_size[0]){
              ...
              if($mode == 'expire'){
                $q = $size['quantity'] + $edit_quantity;
                $_SESSION['error_flash'] = $edit_quantity. 'cart product expired, you can add back to cart';
              }
...
//  $db->query("INSERT INTO wishlist(id,items,username,expire_date) VALUES ({$cart_id}','{$cartQ}','{$user_name}','{$cart_expire}')");
  ...
  if(!empty($updated_items)){
    ...
    foreach ($items as $w_item) {
     ...
     cart_wishlist_update('wishlist',$db,$item,$cart_id,$user_name,$json_update,$cart_expire);
    }
...
function cart_wishlist_update($mode,$db,$item,$cart_id,$user_name,$json_update,$cart_expire){
  $paid = 0;
  if($mode == 'wishlist'){
