...
if(!check_staff_permission('editor')){
  error_redirect('index.php','You do not have clearance to view that page');
}
...
if(isset($_GET['delete'])){
  ...
  header('Location: slide.php');
    ...
    permission_ungranted('slide.php',$message);
  }
}

$dbpath = '';
// if add button is pressed display add slide section else show added slide
  ...
  // store edit id if edit button is clicked
    if(isset($_GET['edit'])){
          ...
          if(isset($_GET['delete_image'])){
            ...
            header('Location: slide.php?edit='.$edit_id);
          }
    ...
    <!-- if edit is clicked set the post option to edit id else set to add slide( add=1)-->
      <form action="slide.php?<?=((isset($_GET['edit']))?'edit='.$edit_id:'add=1');?>" method="POST" enctype="multipart/form-data">
       ...
       <div class="form-group col-md-3">
         ...
         <input type="text" id="url" name="url" class="form-control" value="<?=$linkUrl;?>" placeholder="Enter space for no URL" required>
       </div>
        ...
        <div class="form-group col-md-6">
          <?php if($slide_image != ''): ?>
            ...
            <a href="slide.php?delete_image=1&edit=<?=$edit_id;?>" class="text-danger">Delete Image</a>
          </div>
        <?php else:?>
        ...
        <?php endif; ?>
         ...
         <div class= "form-group pull-right">
           <a href="slide.php" class="btn btn-default">Cancel</a>
...
<a href="slide.php?add=1" class ="btn btn-success pull-right" id="add-product-btn">Add Slides</a><div class="clearfix"></div>
<hr>
<table class="table table-bordered table-condensed table-striped">
  <tbody><th>Action</th><th>Title</th><th>Caption</th><th>Link URL<th>ON/OFF</th></thead>
<tbody>
    <?php while($slide =mysqli_fetch_assoc($slideQ)):?>

    <tr>
        <td>
          <a href ="slide.php?edit=<?=$slide['id'];?>" class= "btn btn-xs btn-info"><span class="glyphicon glyphicon-pencil"></span><a/>
          <a href ="slide.php?delete=<?=$slide['id'];?>" class= "btn btn-xs btn-danger " onclick="return deleletconfirm()"><span class="glyphicon glyphicon-remove"></span><a/>
        </td>
      <td><?=$slide['title'];?></td>
      <td><?=$slide['caption'];?></</td> <!--the money function is in helper class to add the dollar sign -->
      <td><?=$slide['url'];?></td>
      <td><a href = "slide.php?status=<?=(($slide['status']== 0)?'1':'0');?>&id=<?=$slide['id'];?>" class = "btn btn-xs btn-default">
        <span class = "glyphicon glyphicon-<?=(($slide['status']==1)?'minus':'plus');?>"></span>
      </a>&nbsp <?=(($slide['status'] == 1)?'ON':'OFF');?></td>
