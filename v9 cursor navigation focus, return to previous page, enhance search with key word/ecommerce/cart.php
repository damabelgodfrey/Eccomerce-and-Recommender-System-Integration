  ...
  <?php
    foreach($items as $item){
      ...
      foreach($sArray as $sizeString){
        ...
        if($s[0] == $item['size']){
          $available = $s[2]; //third element in array is the available
          $newQ = $available - $item['quantity'];
        }
      }
      ...
      ?>
      <tr>
        <td>[<?=$i;?>] <button class="btn btn-danger btn-xs glyphicon glyphicon-remove" onclick="update_cart('delete','<?=$product['id'];?>','<?=$item['size'];?>','<?=$available;?>');"> Remove</button></td>
        <td><?=$product['title'];?><br>
        ...
        <td>
          ...
          <button class="btn btn-xs btn-info" onclick="update_cart('removeone','<?=$product['id'];?>','<?=$item['size'];?>','<?=$available;?>');"><strong>-</strong></button>
          <?php 
             if ($item['quantity'] >= $available){
                ...
                <?php }else{ ?>
                  <?=$item['quantity'];?>
                  <button class="btn btn-xs btn-info" onclick="update_cart('addone','<?=$product['id'];?>','<?=$item['size'];?>','<?=$available;?>');"><strong>+</strong></button>
