...
if(isset($_GET['Pedit']) && !empty($_GET['Pedit'])){
  if(check_staff_permission('pro')){
    ...
    permission_ungranted('permissions',$message);
  }
}
//delete
if(isset($_GET['Pdelete']) && !empty($_GET['Pdelete'])){
  if(check_staff_permission('pro')){
    ...
    header('Location: permissions'); //redirect bark to the page
  ...
  if(check_staff_permission('pro')){
    ...
    if(!empty($errors)){
      ...
      //add brand to database because no error was caught hence error array is empty
       ...
       $db->query($sql);
       header('Location:permissions'); //reload page
      }
  }else{
...
<p></p><br><br><hr><br>
  ...
  <form class = "form-inline" action="permissions<?=((isset($_GET['Pedit']))?'?Pedit='.$Pedit_id:''); ?>" method="post">
    <div id="myerror">
  ...
  <table class="table table-bordered table-striped table-brand table-condensed table-hover">
    ...
    <tbody>
      ...
      <tr>
        <td><a href="permissions?Pedit=<?=$permission['id']; ?>" class="btn btn-info btn-xs"><span class="glyphicon glyphicon-pencil"></span></a>&nbsp&nbsp
        <a href="permissions?Pdelete=<?=$permission['id']; ?>" class="btn btn-danger btn-xs" onclick="return Pdeleteconfirm()"><span class="glyphicon glyphicon-remove-sign"></span></a></td>
