...
if(isset($_GET['archiveRestore'])){
  if(check_staff_permission('editor')){
    ...
    if (!headers_sent()) {
    header('Location: archiveproducts');
    ?><a href ="products.php" class= "btn btn-danger btn-md" <span class="glyphicon glyphicon-remove"></span><a/><?php
   die('should have redirected by now');
   }
    }else{
      $message = "Please! You do not have sufficient clearance to unarchive product.";
        permission_ungranted('archiveProducts',$message);
      }
}
  ...
  <?php while($archiveProduct =mysqli_fetch_assoc($archive_results)):
    ...
    <tr>
        <td>
          ...
          <a href ="archiveproducts?archiveRestore=<?=$archiveProduct['id'];?>" class= "btn btn-xs btn-default"><span class="glyphicon glyphicon-send"></span><a/>
        </td>
      <td><?=$archiveProduct['title'];?></td>
      <td><?=money($archiveProduct['price']);?></</td> <!--the money function is in helper class to add the dollar sign -->
      <td><?=$category;?></td>
      <td><a href = "archiveProducts?archiveFeatured=<?=(($archiveProduct['featured']== 0)?'1':'0');?>&id=<?=$archiveProduct['id'];?>" class = "btn btn-xs btn-default">
