...
if(isset($_GET['delete'])){
  ...
  permission_ungranted('customer_account');
  }else{
...
if(isset($_GET['add']) || isset($_GET['edit'])){
  $PermissionQuery = $db->query("SELECT * FROM permissions ORDER BY permission"); //get brand from database
...
?>
<div class="box">
<h2 class="text-center"><?=((isset($_GET['edit']))?'Edit ':'Add A New ');?>Staffs</h2><hr>
<form action="customer_account?<?=((isset($_GET['edit']))?'edit='.$edit_id:'add=1');?>" method="post" enctype="multipart/form-data">
...
<?php endif; ?>
  ...
  <div class="form-group col-md-6 text-right" style="margin-top:25px;">
    <a href="customer_account" class="btn btn-default">Cancel</a>
...
$pagConfig = array(
  'baseURL'=>'http://localhost:81/ecommerce/admin/customer_account.php',
  'totalRows'=>$rowCount,
  'perPage'=>$limit
);
...
<form action="customer_account" method="post">
  <div class="form-group col-md-5">
    <input type="text" class="form-control" placeholder="Search customer details.." name="searchProduct" required>
    <button type="submit" class="btn btn btn-lg btn-default"><span class="glyphicon glyphicon-search"></span> Search Customer</button>
  </div>
</form>
</div>
<a href="staff_account" class="btn btn-lg btn-success pull-right" id="add-user-btn">View Staff</a>
</div> <p></p>
<div class="box">
  ...
  <tbody>
    ...
    <tr>
      <td>
        ...
        <?php if($user['email'] != $staff_data['email']){ ?>
          <a href="customer_account?delete=<?=$user['id'];?>" class="btn btn-danger btn-xs"><span class ="glyphicon glyphicon-remove-sign"></span></a>
          <a href="customer_account?edit=<?=$user['id'];?>" class="btn btn-info btn-xs"><span class ="glyphicon glyphicon-pencil"></span></a>
      <?php }else {?>
