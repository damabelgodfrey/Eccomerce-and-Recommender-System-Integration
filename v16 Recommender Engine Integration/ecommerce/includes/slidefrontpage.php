...
//var_dump($slideTitle2);var_dump($url); die();
  ...
  <div id="my-slider1" class="carousel slide" data-ride="carousel">
    ...
    while ($slide = mysqli_fetch_assoc($slideQ)){
      if($slide['flag'] == 1){
      $productQ = $db->query("SELECT id FROM products WHERE (title LIKE '%".$slide['url']."%' OR description LIKE '%".$slide['url']."%' OR p_keyword LIKE '%".$slide['url']."%') AND archive = 0 AND category_activate_flag =1 AND defective_product = 0 LIMIT 1");
      $product = mysqli_fetch_assoc($productQ);
      $product_id = $product['id']; ?>
    <?php } ?>
      <div class="<?=$init?> animation">
                 <?php if (isset($product_id) && $slide['flag'] == 1){?>
                 <img onclick="detailsmodal('add',<?= $product_id ?>)"src="<?=(isset($slide['image'])?$slide['image']:'');?>">
               <?php }else{ ?>
               ...
               <?php } ?>
                   <div class="carousel-caption">
                    ...
                    <!-- <?php if (isset($product_id) && $slide['flag'] == 1){?>
                     <button type ="button" id="dbuttoncarousal" class="btn btn-sm btn-danger" onclick="detailsmodal('add',<?= $product_id ?>)">DETAILS</button>
                     <?php }else{ ?>
