...
public static function getUserMetrix($user_id){
  $type ="User";
  ...
  // id and sim score
public static function computeItemSimilarity($user_id){
   $userItemRatingMatrix = self::getUserMetrix($user_id);
   //LevenshteinDistance, ConsineSimilarity,CosineRatingSimilarityWeighted,CosineSimilarityRatingTagWeighted
   $simAlgorithm ="CosineSimilarityRatingTagWeighted";
  foreach ($userItemRatingMatrix as $id => $rating){
    $ItemPeerGroup = ItemFeatureSimComputation::getFeatureSimCoefficient("ItemBasedCollaborativeFiltering",$simAlgorithm,$userItemRatingMatrix,$id,$rating);
...
//compute summation of Sim*RU1/sum of simmilarity
  public static function computeRatingPrediction($user_id){
    ...
    //remove item predicted but rated by the user.
    ...
    //$finalItemBasedPrediction =array();
   //$finalItemBasedPrediction = array_diff_key($ItemPredictions,$UserRatingMatrix);
