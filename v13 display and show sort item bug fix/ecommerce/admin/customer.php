...
<form action="customer?<?=((isset($_GET['edit']))?'edit='.$edit_id:'add=1');?>" method="post" enctype="multipart/form-data">
...
<a href="users" class="btn btn-lg btn-success pull-right" id="add-user-btn">View Staff</a>
  ...
  <tbody>
    ...
    <tr>
      <td>
        ...
        <?php if($user['email'] != $user_data['email']): ?>
          <a href="customer?delete=<?=$user['id'];?>" class="btn btn-danger btn-xs"><span class ="glyphicon glyphicon-remove-sign"></span></a>
          <a href="customer?edit=<?=$user['id'];?>" class="btn btn-info btn-xs"><span class ="glyphicon glyphicon-pencil"></span></a>
        <?php endif; ?>
      </td>
